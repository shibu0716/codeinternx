...
export default async function LorVerificationPage({ params }: { params: { id: string } }) {
  const result = await verifyDocument(params.id);

  if (!result.isValid || result.document?.type !== 'LOR') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg border-t-4 border-red-500 p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            The Letter of Recommendation ID <strong>{params.id}</strong> could not be verified. It may be invalid or revoked.
          </p>
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 flex items-center justify-center">
      <div className="max-w-2xl w-full bg-white rounded-lg shadow-lg border-t-4 border-[#0A192F] overflow-hidden">
        ...
        <div className="bg-[#0A192F] p-6 text-center text-white">
          <h1 className="text-2xl font-bold tracking-wider">RECOMMENDATION VERIFIED</h1>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8 text-sm">
            <div>
              <p className="text-gray-500 mb-1">LOR ID</p>
            ...
            <div className="col-span-1 sm:col-span-2">
              <p className="text-gray-500 mb-1">Internship</p>
              <p className="font-semibold text-gray-900">{metadata.position || metadata.internship_domain || 'Intern'}</p>
            </div>
        ...
        <div className="bg-gray-50 p-4 border-t text-center text-sm text-gray-500">
          This is a verified authentic Letter of Recommendation issued by CodeInternX.
        </div>
      </div>
    </div>
  );
